...
struct HabitCreationView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 10) {
                TextField("Workout for 15 min", text: $name)
                    .font(.title)
                    .padding(.bottom, 10)
    ...
    func NotificationProperties() -> some View {
        Toggle("Enable reminder notifications", isOn: $enableNotifications)
            .font(.callout)
        ...
        if enableNotifications && isNotificationPermissionGranted {
            DatePicker("Preferred reminder date",
        ...
        if !isNotificationPermissionGranted {
            Text("You need to allow notifications to use this feature.")
                .font(.caption2)
                .foregroundStyle(.gray)
        }
    }
    ...
    func HabitCreationButton() -> some View {
        HStack(spacing: 10) {
            Button(action: createHabit) {
                HStack(spacing: 10) {
                    Text("\(isNewHabit ? "Create" : "Save") habit")
        ...
        Task { @MainActor in
            if let habit {
                ...
                if enableNotifications {
                    ...
                    let habit = Habit(name: name, frequencies: frequencies, notificationIDs: notificationIDs)
                    context.insert(habit)
                } else {
                    let habit = Habit(name: name, frequencies: frequencies, notificationIDs: [])
                    context.insert(habit)
                }
            }
    ...
    private func scheduleNotifications() async throws -> [String] {
        ...
        content.body = "Don't forget to check in on your habit!"
